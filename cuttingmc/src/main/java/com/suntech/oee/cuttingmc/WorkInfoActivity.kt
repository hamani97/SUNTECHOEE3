...
class WorkInfoActivity : BaseActivity() {
    ...
    private fun tabChange(v : Int) {
        if (tab_pos == v) return
        when (tab_pos) {
            1 -> {  btn_work_info_server.setTextColor(ContextCompat.getColor(this, R.color.gray))
                    btn_work_info_server.setBackgroundResource(R.color.tab_off_bg_color)
            }
            2 -> {  btn_work_info_manual.setTextColor(ContextCompat.getColor(this, R.color.gray))
                    btn_work_info_manual.setBackgroundResource(R.color.tab_off_bg_color)
            }
        }
        tab_pos = v
        when (tab_pos) {
            1 -> {  btn_work_info_server.setTextColor(ContextCompat.getColor(this, R.color.white))
                    btn_work_info_server.setBackgroundResource(R.color.tab_on_bg_color)
            }
            2 -> {  btn_work_info_manual.setTextColor(ContextCompat.getColor(this, R.color.white))
